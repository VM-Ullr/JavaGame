...
public class Concentration extends SortBerserker{

	public Concentration(berserker J1) {
		super(J1);
		cooldownOnUse = 4;
		cout = 3;
		portee = 0;
	...
	public void effect(Plateau plate, int x, int y, int k) {
		...
		timer.start();
		onAnimation = true;
		personnage.addBuff(new Buff(2,0,0,2,0,0,0));
	...
	public String [] lancerSort( Plateau plate, int x, int y){
		...
		if(this.isUsable() ){
			...
			if(personnage.isReussite()){
				...
				this.used();
				s[0]="reussite";
				s[1]="vous gagnez 2 de chance";
			}else{
				personnage.setPA(personnage.getPA()-cout);
				s[0]="echec";
				s[1]="vous etes dissipes";
			}
		}
		s[3]=s1[0];
		s[4]=s1[1];
