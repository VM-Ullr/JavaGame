...
public class Rage extends SortPaladin{

	public Rage(paladin J1) {
		super(J1);
		cooldownOnUse = 4;
		cout = 2;
		portee = 0;
	...
	public void effect(Plateau plate, int x, int y, int k) {
		...
		timer.start();
		onAnimation = true;
		personnage.addBuff(new Buff(2,20,0,0,0,0,0));
	...
	public String [] lancerSort(Plateau plate, int x, int y){
		...
		if(this.isUsable() && !this.isOnCooldown()){
			...
			if(personnage.isReussite()){
				...
				this.used();
				s[0]="reussite";
				s[1]="vous gagnez 20 de force";
			}else{
				personnage.setPA(personnage.getPA()-cout);
				s[0]="echec";
				s[1]="vous n'avez pas vraiment l'air enrage...";
			}
		}
		s[3]=s1[0];
		s[4]=s1[1];
