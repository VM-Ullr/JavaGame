...
public class EsquiveDeLaPolice extends SortVoleur{

	public EsquiveDeLaPolice(voleur J1) {
		super(J1);
		cooldownOnUse = 6;
		cout = 4;
		portee = 0;
		type = "Intelligence";
		nom = "Esquive de la police";
		delay = 100;
	...
	public void effect(Plateau plate, int x, int y, int k) {
		...
		timer.start();
		onAnimation = true;
		personnage.addBuff(new Buff(1,0,0,0,0,100-personnage.getDefBuff(),0));
	...
	public String [] lancerSort(Plateau plate, int x, int y){
		...
		if(this.isUsable()){
			...
			if(personnage.isReussite()){
				...
				this.used();
				s[0]="reussite";
				s[1]="vous esquivez la police hehe !";
			}else{
				personnage.setPA(personnage.getPA()-cout);
				s[0]="echec";
				s[1]="la police vous chope ! ";
			}
		}
		s[3]=s1[0];
		s[4]=s1[1];
